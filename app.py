...
def generar_dataset(n, semilla, pct_na_temp, pct_na_hum, n_outliers):
    ...
    tipos_sensor = rng.choice(
        ["DHT22", "BMP180", "LDR"],
        size=n,
        p=[0.5, 0.3, 0.2]
    )

    ubicaciones = rng.choice(
        ["invernadero_1", "patio", "bodega"],
        size=n
    )

    calidad_senal = rng.choice(
        ["baja", "media", "alta"],
        size=n,
        p=[0.1, 0.3, 0.6]
    )
    ...
    if n_outliers > 0:
        idx = rng.choice(
            df.index,
            size=min(n_outliers, n),
            replace=False
        )

        df.loc[idx, "temperatura"] = rng.choice(
            [-40, 95, 120],
            size=len(idx)
        )
...
def get_df():
    """
    Dataset base compartido entre todas las páginas
    mediante session_state.
    """
    ...
    if (
        st.session_state.get("_cfg") != cfg
        or "df_base" not in st.session_state
    ):
...
st.sidebar.caption(
    "Sensores IoT sintéticos "
    "(temperatura, humedad, presión, lecturas/hora)"
)
...
st.session_state["semilla"] = st.sidebar.number_input(
    "Semilla aleatoria",
    value=42,
    step=1
)

st.session_state["pct_na_temp"] = st.sidebar.slider(
    "% missing en temperatura",
    0,
    30,
    5
)

st.session_state["pct_na_hum"] = st.sidebar.slider(
    "% missing en humedad",
    0,
    30,
    4
...
st.session_state["n_outliers"] = st.sidebar.slider(
    "N° de outliers inyectados (temperatura)",
    0,
    30,
    6
)
...
if pagina == "🏠 Inicio":
    ...
    st.markdown("""
    Antes de construir cualquier modelo o método computacional,
    es necesario entender, limpiar y estructurar los datos disponibles.

    Esta aplicación permite experimentar con conceptos de preparación
    de datos utilizando un dataset sintético de sensores IoT.

    Usa el menú de la izquierda para:

    - Configurar el dataset.
    - Navegar por las 5 secciones del módulo.
    - Analizar missing values y outliers.
    - Comparar normalización y estandarización.
    - Comparar diferentes tipos de división de datos.
    - Analizar correlación y covarianza.
    """)
...
# ============================================================
    ...
    st.markdown("""
    Cada columna del dataset representa un tipo distinto:
    numérico continuo, numérico discreto, categórico nominal,
    categórico ordinal o temporal.
    """)
    ...
    with col1:
        ...
        st.subheader(
            "Tipos originales (detectados por pandas)"
        )
    ...
    with col2:
        ...
        if aplicar:
            ...
            df_conv["tipo_sensor"] = (
                df_conv["tipo_sensor"].astype("category")
            )

            df_conv["ubicacion"] = (
                df_conv["ubicacion"].astype("category")
            )
            ...
            df_conv["calidad_senal"] = pd.Categorical(
                df_conv["calidad_senal"],
                categories=orden,
                ordered=True
            )

        st.dataframe(
            df_conv.dtypes.astype(str).rename("dtype"),
            use_container_width=True
        )

        mem_despues = (
            df_conv.memory_usage(deep=True).sum() / 1024
        )

        st.metric(
            "Memoria total (después)",
            f"{mem_despues:.2f} KB",
            delta=f"{mem_despues - mem_antes:.2f} KB"
        )

    st.info(
        "💡 `calidad_senal` es una categoría ordinal "
        "(baja < media < alta). "
        "`ubicacion` y `tipo_sensor` son nominales."
    )
...
# ============================================================
    ...
    if metodo_imputacion == "Interpolación lineal":
        ...
        df_tratado = df_tratado.dropna(
            subset=["temperatura", "humedad"]
        )

    elif metodo_imputacion == "Imputar con la media":

        df_tratado["temperatura"] = (
            df_tratado["temperatura"]
            .fillna(df_tratado["temperatura"].mean())
        )

        df_tratado["humedad"] = (
            df_tratado["humedad"]
            .fillna(df_tratado["humedad"].mean())
        )

    st.metric(
        "Missing restantes tras el tratamiento",
        int(
            df_tratado[
                ["temperatura", "humedad"]
            ].isna().sum().sum()
    ...
    axes[1].set_title(
        f"{variable} — sin outliers detectados"
    )
...
# ============================================================
    ...
    st.markdown(
        ...
        "el dataset completo es una matriz. "
        "Aquí puedes comparar el efecto de cada transformación."
    )
    ...
    c1.metric(
        "Mínimo",
        f"{transformada.min():.3f}"
    )

    c2.metric(
        "Máximo",
        f"{transformada.max():.3f}"
    )

    c3.metric(
        "Media",
        f"{transformada.mean():.3f}"
    )

    c4.metric(
        "Desv. estándar",
        f"{transformada.std():.3f}"
    )
...
# ============================================================
    ...
    if pct_test < 0:
        ...
        st.error(
            "La suma de Train + Validation supera el 100%. "
            "Ajusta los sliders."
        )
        ...
        if tipo_split == "Cronológica":
            ...
            train = df_ordenado.iloc[
                :n_train
            ]

            val = df_ordenado.iloc[
                n_train:n_train + n_val
            ]

            test = df_ordenado.iloc[
                n_train + n_val:
            ]
        ...
        if tipo_split == "Cronológica":
            ...
            st.info(
                "📌 Fecha de corte Train → Validation: "
                f"**{val['timestamp'].min()}**"
            )
